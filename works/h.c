...
// BLUE //
      ...
      if (x < 400) {
        ...
        // n_index--;
        fputc(normalized_input[n_index] + (float)exp(theta/140.0) , tga);
      }
...
// GREEN //
      ...
      fputc( green_input<<2, tga);
      // fputc(normalized_input[n_index-HEIGHT-1] + (float)log(theta/60.0)*y/52.0, tga);

// RED //
      if (y < 200 && y > 155) {
        n_index--;
        // fputc(normalized_input[(n_index+300)] + (float)exp((theta*5.0)/40.0) - (float)log((theta*5.0)/40.0), tga);
        fputc(normalized_input[(n_index+300)] + (float)exp((theta*3.0)/40.0) - (y+300)/4.0, tga);
        n_index++;
      } else {
