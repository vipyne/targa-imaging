...
int main (int argc, char* argv[])
{
  ...
  for (int y = 0; y < HEIGHT; ++y)
  {
    for (int x = 0; x < WIDTH; ++x)
    {
      ...
      if (n_index > WIDTH/2) 
      {
        fputc(normalized_input[n_index-WIDTH/5] + somethingelse + (float)log(theta/25.0), tga);
      } else {
        fputc(normalized_input[n_index] + y + (float)log(theta/20.0), tga);
      }

      fputc(normalized_input[n_index] + (float)log(theta), tga);
      
      if (x + y < HEIGHT/2)
      {
        fputc(normalized_sorted[input_binary_length - n_index] + (float)sin(theta/11.0)*x, tga);
      } else {
        fputc(normalized_sorted[n_index + n_index] + (float)cos(theta/10.0)*y, tga);
      }
